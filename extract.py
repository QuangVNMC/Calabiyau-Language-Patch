...
def check_cryptography():
    try:
        ...
        print()
        print("Please install cryptography using:")
...
def parse_pak(pak_path, aes_key=None):
    ...
    with open(pak_path, 'rb') as f:
        ...
        for _ in range(entry_count):
            ...
            if ie_method != 0:
                ie_block_count = struct.unpack_from(
                    '<I', decrypted, ipos
                )[0]
...
def main():
    ...
    print(r"Example path: D:\Games\Strinova\Game\PM\Content\Paks\Game_Localization_001_P.pak")
    pak_path = input("Enter path to Game_Localization_001_P.pak file: ").strip().strip("'\"")
    ...
    print(
        f"\nParsing {os.path.basename(pak_path)} "
        f"({os.path.getsize(pak_path) / 1024 / 1024:.1f} MB)..."
    )
    ...
    choice = input(
        f"\nSelect language "
        f"(1-{len(all_langs)}, or comma-separated list, or 'all'): "
    ).strip()
    ...
    print(
        f"\nExtracting {len(selected)} language(s) "
        f"to {out_dir}/"
    )
    ...
    for lang in selected:
        print(
            f"\n  [{lang}] "
            f"{get_lang_name(lang)}:"
        )

        entry = locres_entries[lang]

        try:
            data = extract_entry_from_pak(
                pak_path,
                entry,
                aes_key
            )
